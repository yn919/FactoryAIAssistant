...
1. Unityが起動時に `SensorLoop` を開始する
2. 5秒ごとに `/sensor` を呼び出す
...
現在の実装では、`Random.Range` によって値を生成しており、バックエンドAPIと連携する前の簡易デモとして利用されます。
...
- APIが応答しない場合、出力はエラーメッセージを表示し、アプリが停止しないように設計されている
- センサAPIが失敗してもUIスクリプトのメインフローは停止しないように処理している
...
- 実設備データを取得するデータソースの導入
- 監視カレンダとアラート機能の追加
- 会話履歴の保存と再利用
- AI応答時の安全策やファクトチェック
- モバイルや組込向けにUIレイアウトの再設計

## 12. まとめ

本Unityモジュールは、工場設備の可視化とAIアシスタント連携を体験的に確認できるHMIクライアントとして構成されています。FastAPI側のAPIと組み合わせることで、実際の運用に近い対話型インターフェースのプロトタイプとして利用できます。
